...
from nanochat.vision import VisionConfig, VisionTransformer, VisionProjector, DeepSeekVisionTransformer
...
class GPTConfig:
    ...
    # DeepSeek Vision params
    vision_backbone: str = "vit" # "vit" or "deepseek"
    vision_deepseek_path: str = None
    ...
    def __init__(self, config):
        ...
        if config.use_visual_tokens:
            ...
            vision_cfg = VisionConfig(
                ...
                deepseek_local_path=config.vision_deepseek_path
            )

            if config.vision_backbone == "deepseek":
                try:
                    # DeepSeek handles its own projection to n_embd
                    self.vision_encoder = DeepSeekVisionTransformer(vision_cfg, llm_embed_dim=config.n_embd)
                    self.vision_projector = nn.Identity()
                    print0("Initialized DeepSeekVisionTransformer")
                except ImportError as e:
                    print0(f"Failed to load DeepSeekVisionTransformer: {e}. Falling back to ViT.")
